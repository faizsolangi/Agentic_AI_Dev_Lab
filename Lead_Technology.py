...
def generate_email(name, industry):
    ...
    template = """
    Subject: Free 15-Min Automation Workflow Audit

    Hi {name},
    As a techologist in the {industry} space, you could save hours weekly with AI automation.
    I’d love to offer you a free 15-min audit to optimize your processes.
    Best,
    [Your Name]
    solinnovate.io
    """
    prompt = PromptTemplate(template=template, input_variables=["name", "technology"])
...
def score_leads(leads):
    scorer = Agent(
        role="Lead Scorer",
        goal="Score technology leads based on Apollo data",
        backstory="Expert in evaluating technology professionals",
        llm=ChatOpenAI(model="gpt-4o-mini", openai_api_key=os.getenv("OPENAI_API_KEY"))
    )
    task = Task(
        description="Score leads based on industry and title: +10 for 'Technologist' titles, +5 for 'Test Lead' or 'QA Leads' roles, +2 for technology industry",
    ...
    for lead in leads:
        ...
        base_score = 2 if industry == "Technology" else 0
        title_score = 10 if "technology" in title else 5 if "Test Lead" in title or "QA Lead" in title else 0
...
# Streamlit dashboard
def run_dashboard():
    st.title("Outreach Bot Dashboard")
    ...
    if st.button("Refresh Leads and Send Emails"):
        ...
        for lead in scored_leads:
            if lead["email"]:
                email_content = generate_email(lead["name"], "Technology")
